...
function updateServiceInfo() {
    fetch('/api/service-updates')
        ...
        .then(updates => {
            ...
            if (updates.length === 0) {
                scroller.innerHTML = '<span class="service-message">GO Transit - All services operating normally</span>';
            } else {
                scroller.innerHTML = updates.map(update => 
                    `<span class="service-message">${update.line}: ${update.status} - ${update.details}</span>`
                ).join(' • ');
            }
            startServiceScrolling();
        })
        .catch(err => {
            console.error('Error fetching service updates:', err);
            const scroller = document.getElementById('serviceScroller');
            scroller.innerHTML = '<span class="service-message">GO Transit - Service information currently unavailable</span>';
        });
}

let isServiceScrolling = false;
const SCROLL_SPEED = 0.03; // Lower = faster

function startServiceScrolling() {
    if (isServiceScrolling) return;
    
    const scrollingText = document.querySelector('.service-message');
    if (!scrollingText) return;
    
    scrollingText.style.transform = 'translateX(100%)';
    
    const textWidth = scrollingText.offsetWidth;
    const containerWidth = document.querySelector('.service-scroller').offsetWidth;
    ...
    setTimeout(() => {
        ...
        isServiceScrolling = true;
        scrollingText.addEventListener('transitionend', resetServiceScrolling);
    }, 100);
}

function resetServiceScrolling() {
    const scrollingText = document.querySelector('.service-message');
    if (!scrollingText) return;
    ...
    scrollingText.removeEventListener('transitionend', resetServiceScrolling);
    
    setTimeout(startServiceScrolling, 1000);
    isServiceScrolling = false;
}

// Update service info every 5 minutes
updateServiceInfo();
setInterval(updateServiceInfo, 300000);
